...
##    def getCT(self): return self.__ct
    ...
    def Biplot(self, dim = (0, 1)):
        ...
        pylab.plot(self.getPrincipalRowProfilesCoordinates()[:, dim[0]], self.getPrincipalRowProfilesCoordinates()[:, dim[1]], 'wo',
            self.getPrincipalColProfilesCoordinates()[:, dim[0]], self.getPrincipalColProfilesCoordinates()[:, dim[1]], 'wo', alpha=0.0)
        if self.labelR:
            # index
            for i, x, y in zip(range(len(self.getPrincipalRowProfilesCoordinates()[:, dim[0]])), \
                ...
                pylab.text(x, y, self.labelR[i], horizontalalignment='center', color="0.25", size="x-small")
        if self.labelC:
            # columns
            for i, x, y in zip(range(len(self.getPrincipalColProfilesCoordinates()[:, dim[0]])), \
                ...
                pylab.text(x, y, self.labelC[i], horizontalalignment='center', weight="bold", size="x-small", color="r")                
        pylab.grid()
        pylab.show()                
